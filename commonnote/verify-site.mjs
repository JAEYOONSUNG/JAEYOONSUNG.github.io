...
const requiredFiles = [
  ...
  "assets/motion/crimson-story-en.mp4",
  "assets/motion/crimson-story-ko.mp4",
  "assets/motion/crimson-story-en-poster.jpg",
  "assets/motion/crimson-story-ko-poster.jpg",
...
for (const { file, lang, canonical, html } of pages) {
  ...
  check(/\.\/styles\.css\?v=crimson-balance-v3/.test(html), `${file}: stylesheet must use the current cache-busting release key`);
  check(/\.\/site\.js\?v=crimson-balance-v3/.test(html), `${file}: interaction script must use the current cache-busting release key`);
...
check(/<video[^>]+muted[^>]+loop[^>]+playsinline/.test(landing), "English landing must include a muted inline video loop");
check(/<video[^>]+muted[^>]+loop[^>]+playsinline/.test(landingKo), "Korean landing must include a muted inline video loop");
...
check(/living-glyphs\.js\?v=crimson-balance-v3/.test(landing) && /living-glyphs\.js\?v=crimson-balance-v3/.test(landingKo), "both landing pages must cache-bust the living glyph renderer");
